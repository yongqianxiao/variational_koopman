...
class VariationalKoopman():
    def __init__(self, args):
        ...
        self.x = tf.Variable(np.zeros((2 * args.batch_size * args.seq_length, args.state_dim), dtype=np.float32),
                             trainable=False, name="state_values")
        self.u = tf.Variable(np.zeros((args.batch_size, 2 * args.seq_length - 1, args.action_dim), dtype=np.float32),
                             trainable=False, name="action_values")

        # Placeholders for values needed for ilqr
        self.u_ilqr = tf.Variable(np.zeros((args.batch_size, args.seq_length, args.action_dim), dtype=np.float32),
                                  trainable=False, name="action_values_ilqr")
    ...
    def _create_temporal_encoder(self, args):
        ...
        padded_u = tf.concat([tf.zeros([args.batch_size, 1, args.action_dim]), self.u[:, :(args.seq_length - 1)]],
                             axis=1)
    ...
    def _infer_observations(self, args):
        ...
        self.g_dists = tf.reshape(tf.stack(self.g_dists, axis=1),
                                  [args.batch_size * args.seq_length, 2 * args.latent_dim])
    ...
    def _create_prior_network(self, args):
        ...
        u_reshape = tf.reshape(self.u[:, :(args.seq_length - 1)],
                               [args.batch_size * (args.seq_length - 1), args.action_dim])
        ...
        g1_prior = tf.concat(
            [tf.zeros([args.batch_size, 1, args.latent_dim]), tf.ones([args.batch_size, 1, args.latent_dim])], axis=2)
    ...
    def _propagate_solution(self, args):
        ...
        self.A_inv = tf.matrix_solve_ls(Y - tf.matmul(self.u[:, :(args.seq_length - 1)], self.B), self.g_vals[:, :-1],
    ...
    def _generate_predictions(self, args):
        ...
        self.x_future_norm = tf.reshape(self._get_decoder_output(args, z_pred),
    ...
    def _get_cost(self, args, z_u_t):
        ...
        if args.domain_name == 'Pendulum-v0':
            return tf.square(tf.atan2(states[:, 1], states[:, 0])) + 0.1 * tf.square(states[:, 2]) + 0.001 * tf.square(
                tf.squeeze(u_t))
        else:
    ...
    def _find_ilqr_params(self, args):
        ...
        states_pred = self._get_decoder_output(args,
                                               tf.reshape(self.xs, [-1, args.latent_dim])) * self.scale + self.shift
